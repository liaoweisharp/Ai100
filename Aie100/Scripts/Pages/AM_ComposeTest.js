...
/// <reference path="AM_SelectQuestions.js" />
...
function showComposeContent() {
    ...
    if (!_BookStructureData) {
        ...
        $excuteWS("~CmsWS.getBookStructureArray", { isbn: SimpleUser.user.isbn, isLazy: true }, AM_buildBookStructureTree, null, null);
    } else {
        if ($("#bookStructureTree").data("_isNewLoad") == true) {   //每次新打开组卷界面层的时候重新加载树，以清除上一次在树上的选择
            $("#bookStructureTree").data("_isNewLoad", false);
            AM_buildBookStructureTree(_BookStructureData);
        }
    }
}

function AM_buildBookStructureTree(bsData) {
    ...
    var $tree = $("#bookStructureTree").dynatree({
        ...
        onLazyRead: function (node) {
            ...
            $excuteWS("~CmsWS.getLearningObjectiveWithStructureList", { structureId: structureId, isbn: SimpleUser.user.isbn, exCurriculumId: "", userExtend: SimpleUser.user }, onGetKnowledgePoints, null, node);
        },
...
function onBuildTest() {
    ...
    $excuteWS("~CmsWS.getQuestionListByTestSample", { testSamples: TestSampleArray, structureIds: structureIds, loIds: loIds, difficulty: diff, userExtend: SimpleUser.user }, function (result) {
...
function showByLo() {
    ...
    $excuteWS("~CmsWS.getLoQuestionArray", { questionIds: questionIds, userExtend: SimpleUser.user }, function (result) {
        $content.hideLoading();
...
function replaceQuestion(questionId) {
    ...
    if (q.parentFlag == "1") {  //如果是母题还需返回子题
        $excuteWS("~CmsWS.getSubQuestionListById", { questionId: q.id, qpvSeedId: q.qpvSeedId, usersExtendWrapper: SimpleUser.user }, function (result) {
            if (result) {
...
function autoSelectQuestion(questionId, replaceRow) {
    var testQuestionTypeId = $("#dvTestQuesType li.selected").attr("id");
    var $content = $("#dvQuesBox").showLoading();
    $excuteWS("~CmsWS.getQuestionListByQuestion", { questionId: questionId, testQuestionTypeId: testQuestionTypeId, userExtend: SimpleUser.user }, function (result) {
        $content.hideLoading();
...
function manualSelectQuestion(questionId, replaceRow) {
    ...
    $excuteWS("~CmsWS.getQuestionListByStructureAndLo", { testQuestionTypeId: testQuestionTypeId, structureIds: structureIds, loIds: loIds, questionIds: quesIds, userExtend: SimpleUser.user }, function (result) {
        $content.hideLoading();
        if (result && result.length > 0) {
...
function saveTestQuestion() {
    ...
    $excuteWS("~CmsWS.saveTestQuestion", { test: testInfo, questions: _TestQuestionBank, userExtend: SimpleUser.user }, function (result) {
        if (result) {
            closeBuildTestBox();
            queryTestInfo();
            $("#tbTestList").data("_testQuestionId", result);
