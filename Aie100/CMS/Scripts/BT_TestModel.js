...
function onSelectTestModel() {
    ...
    if (TestModelArray.length > 0) {
        ...
        var bookId = $("#selBookList").find("option:selected").attr("id");
        $excuteWS("~CmsWS.getTestModelList", { bookId: bookId, userExtend: SimpleUser }, function (result) {
            $dvTestModelList.hideLoading();
...
function applyTestModel(testModelId, obj) {
    ...
    if (testModel) {
        ...
        $excuteWS("~CmsWS.getTestSampleListByModel", { testModelId: testModelId, userExtend: SimpleUser }, function (result) {
